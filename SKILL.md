...
- If the saved Origin theme already contains acceptable point-line colors, line styles, symbols, and symbol fill behavior, prefer `figures[].theme_controls_series: true`; then only adjust axes, legend, and export settings. Do not duplicate theme-owned curve styling in JSON unless the theme result is visibly wrong.
- Symbol edge and fill colors should match the corresponding line color by default. Set symbol shape/interior first, then reapply line/symbol edge color and symbol fill color so graph themes cannot leave all symbols black. When exact symbol fill matching matters, prefer `figures[].origin_color_indices` over direct RGB colors because Origin's symbol fill commands are more reliable with palette indices.
- For multi-curve line+symbol plots, ungroup the plots before per-curve styling when `symbols`, `line_styles`, or `origin_color_indices` are used; grouped plot formatting can override individual symbol colors.
